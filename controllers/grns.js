...
exports.createGRN = async (req, res) => {
  try {
    ...
    if (grn)
      return res.status(400).send(`GRN ${req.body.grnNo} Already Created`);
    ...
    if (req.body.paymentType == "Credit") {
      const supplier = await User.findById(req.body.supplier).select(
        "-password"
      );
      if (supplier.credit) {
      ...
      if (supplier.projects) {
        if (!supplier.projects.includes(req.body.project))
          supplier.projects.push(req.body.project);
      }
    ...
    req.body.grnsList.forEach((i) => {
      Item.findOne({ name: i.description }).then((item) => {
        item.quantity = parseInt(item.quantity) + parseInt(i.deleveredQty);

        if (item.projects) {
          if (!item.projects.includes(req.body.project))
            item.projects.push(req.body.project);
        }
        item.projects = [req.body.project];

        if (item.suppliers) {
          if (!item.suppliers.includes(req.body.supplierName))
            item.suppliers.push(req.body.supplierName);
        }
    ...
    if (project.suppliers) {
      if (!project.suppliers.includes(req.body.supplierName))
        project.suppliers.push(req.body.supplierName);
    }
    ...
    console.error("Error (Create GRN) : ", error);
...
exports.getAllGRNs = async function (req, res) {
  try {
    ...
    if (grns.length === 0) return res.status(404).send("No GRNs Found");

    res.status(200).send(grns);
  } catch (error) {
    console.error("Error (Get All GRNs) : ", error);
    res.status(500).send(error);
  }
};
...
exports.getOneGRN = async function (req, res) {
  try {
    ...
    if (!grns) return res.status(404).send("GRN Not Found");

    res.status(200).send(grns);
  } catch (error) {
    console.error("Error (Get One GRN) : ", error);
    res.status(500).send(error);
  }
};
